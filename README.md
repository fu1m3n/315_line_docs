...
Альтернативным способом подключения SCADA-системы к установке - использование протокола OPC DA и стороннего сервера. Для этого используются программы [OPC DA Server](https://github.com/fu1m3n/315_line_docs/blob/main/OPC%20DA%20Server.exe) и [MatrikonExplorer](https://github.com/fu1m3n/315_line_docs/blob/main/matrikonopcexplorer.exe), их можно запустить на Windows.

После подключения компьютера с установленными программами к установке, необходимо изменить IP компьютера, чтобы он был в той же подсети, что и установка. Можно использовать свободный IP адрес, например, 10.1.1.11. С помощью команды `ping IP_ADDRESS_HERE` в консоли, можно проверить работоспособность, должен совершаться обмен пакетами.
...
Чтобы следить за изменением значений переменных можно использовать программу MatrikonExplorer, хотя это не обязательно.

Необходимо выбрать созданный сервер, подключиться к нему, нажать на кнопку "Add Tags", перетащить все доступные теги из папок (по отдельности для каждой папки), которые были созданы в предыдущем шаге в Topic Definition.
