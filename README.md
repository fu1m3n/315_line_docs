...
- Провести ревизию комплектующих для линии и составить перечень недостающих элементов;
- Создать SCADA систему для визуализации работы и взаимодействия с установкой;
...
### Введение
Код программы на языке Python находится [здесь](https://github.com/fu1m3n/315_line_docs/blob/main/OPC%20UA%20server%20for%20EasyIP).<br>
...
Также в качестве информационного ресурса и основы для парсера использовался код открытой библиотеки EasyIP на Python, находящийся в репозитории по ссылке https://github.com/birchroad/fstlib/tree/master/src.
...
Формат заголовка: `HEADER_FORMAT = "<B B H H B B H H B B H H H"`.</br>
...
Функция работает в бесконечном цикле, постоянно опрашивая контроллер.<br>

- `with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:` — создаёт сокет для работы с UDP портом.

Затем в цикле `for` осуществляется перебор требуемых типов данных, для которых будут формироваться запросы. Потом формируется запрос и отправляется в соответствии с адресом и портом контроллера.<br>
